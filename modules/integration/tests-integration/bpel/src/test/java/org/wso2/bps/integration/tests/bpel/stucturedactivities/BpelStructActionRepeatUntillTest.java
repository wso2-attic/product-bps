...
public class BpelStructActionRepeatUntillTest extends BPSMasterTest {

    private static final Log log = LogFactory.getLog(BpelStructActionRepeatUntillTest.class);
    ...
    public void deployArtifact() throws InterruptedException, RemoteException, MalformedURLException,
        ...
        setEnvironment();
        uploadBpelForTest("TestRepeatUntil");
        requestSender.waitForProcessDeployment(backEndUrl + "TestRepeatUntilService");
    }

    @Test(groups = {"wso2.bps", "wso2.bps.structures"}, description = "Deploys TestRepeatUntil with activity", priority = 0)
    public void runSuccessCase() throws Exception {
        int instanceCount = 0;

        String processID = bpelProcessManagementClient.getProcessId("TestRepeatUntil");
    ...
    public void cleanup() throws PackageManagementException, InterruptedException, RemoteException,
        ...
        bpelPackageManagementClient.undeployBPEL("TestRepeatUntil");
